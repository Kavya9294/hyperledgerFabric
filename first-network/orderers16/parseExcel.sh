...
docker logs peer3.org1.example.com >& peer31.txt | sed -e '1,9d' < peer31.txt >>temp_peer0.txt
...
docker logs peer3.org2.example.com >& peer32.txt | sed -e '1,9d' < peer32.txt >>temp_peer0.txt
...
cat peer_sample1.txt > peer_config16Orderer.csv
...
cat sample1.txt > config16Orderer.csv
